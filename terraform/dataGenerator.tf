  ...
  user_data = templatefile("dataGenerator.tftpl",
    {
      ...
      KAFKA_EXECUTOR_HOST = var.kafka_populate ? aiven_kafka.kafka1[0].components[0].host : "x"
      KAFKA_EXECUTOR_PORT = var.kafka_populate ? aiven_kafka.kafka1[0].components[0].port : "0"

      KAFKA_EXECUTOR_SCHEMA_REGISTRY_HOST     = var.kafka_populate ? aiven_kafka.kafka1[0].components[6].host : "x"
      KAFKA_EXECUTOR_SCHEMA_REGISTRY_PORT     = var.kafka_populate ? aiven_kafka.kafka1[0].components[6].port : "0"
      KAFKA_EXECUTOR_SCHEMA_REGISTRY_USER     = var.kafka_populate ? aiven_kafka.kafka1[0].service_username : "x"
      KAFKA_EXECUTOR_SCHEMA_REGISTRY_PASSWORD = var.kafka_populate ? aiven_kafka.kafka1[0].service_password : "x"


      KAFKA_EXECUTOR_OPENSEARCH_ENABLE   = var.os_populate
      KAFKA_EXECUTOR_OPENSEARCH_HOST     = var.os_populate ? aiven_opensearch.os1[0].service_host : "x"
      KAFKA_EXECUTOR_OPENSEARCH_PORT     = var.os_populate ? aiven_opensearch.os1[0].service_port : "0"
      KAFKA_EXECUTOR_OPENSEARCH_USER     = var.os_populate ? aiven_opensearch.os1[0].service_username : "x"
      KAFKA_EXECUTOR_OPENSEARCH_PASSWORD = var.os_populate ? aiven_opensearch.os1[0].service_password : "x"
